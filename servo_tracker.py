...
class ServoTracker:
    ...
    def __init__(
        ...
        tilt_steps = int(tilt_travel_deg * STEPS_PER_DEG)
        self._pan_min  = max(MIN_POS, PAN_HOME  - pan_steps)
        self._pan_max  = min(MAX_POS, PAN_HOME  + pan_steps)
        self._tilt_min = max(MIN_POS, TILT_HOME - tilt_steps)
        self._tilt_max = min(MAX_POS, TILT_HOME + tilt_steps)
    ...
    def shake(self, count: int = 5, amplitude_deg: float = 20.0) -> None:
        """Rapidly oscillate pan servo left-right (67 gesture response)."""
        threading.Thread(
